...
oneTimeSetUp(){
	...
	if [ -z "${STAGING_IP}" ]; then
		STAGING_IP=http://83.212.118.152
	fi
...
testClusterCreate(){
	...
	if [ "$DO_INTEGRATION_TEST" = true ]; then
		...
		declare -a ARR_RESULT=($(orka create ecosystem_integration_test 2 4 6144 20 4 6144 20 standard escience.grnet.gr 1 128 --use_hadoop_image Ecosystem-on-Hue-3.8.0))
