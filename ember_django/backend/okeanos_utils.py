...
def destroy_server(token, id):
    ...
    auth = check_credentials(unmask_token('encrypt_token',token))
    current_task.update_state(state="Authenticated")
    set_server_state(token, id, 'Deleting VRE server and its public IP')
    endpoints, user_id = endpoints_and_user_id(auth)
    cyclades = init_cyclades(endpoints['cyclades'], unmask_token('encrypt_token',token))
    nc = init_cyclades_netclient(endpoints['network'], unmask_token('encrypt_token',token))
