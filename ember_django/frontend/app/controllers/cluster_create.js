...
App.ClusterCreateController = Ember.Controller.extend({
	...
	}, // Small Medium and Large predefined flavors
	reverse_storage_lookup : {
		'ext_vlmc' : 'Archipelago',
		'drbd' : 'Standard'
	},
	...
	projects_av : function() {
		...
		for (var i = 0; i < length; i++) {
			...
			project['quotas'] = space_separate_project_name_and_quota + 'VMs:' + model.objectAt(i).get('vms_av').length + space_between_quota + 'CPUs:' + model.objectAt(i).get('cpu_av') + space_between_quota + 'RAM:' + model.objectAt(i).get('ram_av') + 'MB' + space_between_quota + 'Disk:' + model.objectAt(i).get('disk_av') + 'GB';
	...
	vm_flavor_buttons_response : function() {
		...
		for ( index1 = 0; index1 < this.number_of_flavor_sizes; index1++) {
			for ( index2 = 0; index2 < this.number_of_roles; index2++) {
				if ((this.flavor_settings[this.list_of_flavor_sizes[index1]]['cpu'] == this.get(this.list_of_roles[index2] + '_cpu_selection')) && (this.flavor_settings[this.list_of_flavor_sizes[index1]]['ram'] == this.get(this.list_of_roles[index2] + '_ram_selection')) && (this.flavor_settings[this.list_of_flavor_sizes[index1]]['disk'] == this.get(this.list_of_roles[index2] + '_disk_selection'))) {
	...
	vm_flavor_buttons : function() {
		...
		for (var i = 0; i < length; i++) {
			...
			if ((this.get('vm_flavor_selection_master') !== undefined) && (this.get('vm_flavor_selection_master') !== null) && (this.get('vm_flavor_selection_master') !== '')) {
				...
				if ((this.get('master_cpu_selection') == this.flavor_settings['Small']['cpu']) && (this.get('master_ram_selection') == this.flavor_settings['Small']['ram']) && (this.get('master_disk_selection') == this.flavor_settings['Small']['disk'])) {
					vm_flavor_newMaster_Id = "master_vm_flavors_Small";
					choice = document.getElementById(vm_flavor_newMaster_Id);
					choice.style.color = "white";
				}
				if ((this.get('master_cpu_selection') == this.flavor_settings['Medium']['cpu']) && (this.get('master_ram_selection') == this.flavor_settings['Medium']['ram']) && (this.get('master_disk_selection') == this.flavor_settings['Medium']['disk'])) {
					vm_flavor_newMaster_Id = "master_vm_flavors_Medium";
					choice = document.getElementById(vm_flavor_newMaster_Id);
					choice.style.color = "white";
				}
				if ((this.get('master_cpu_selection') == this.flavor_settings['Large']['cpu']) && (this.get('master_ram_selection') == this.flavor_settings['Large']['ram']) && (this.get('master_disk_selection') == this.flavor_settings['Large']['disk'])) {
		...
		for (var i = 0; i < length; i++) {
			...
			if ((this.get('vm_flavor_selection_slaves') !== undefined) && (this.get('vm_flavor_selection_slaves') !== null) && (this.get('vm_flavor_selection_slaves') !== '')) {
				...
				if ((this.get('slaves_cpu_selection') == this.flavor_settings['Small']['cpu']) && (this.get('slaves_ram_selection') == this.flavor_settings['Small']['ram']) && (this.get('slaves_disk_selection') == this.flavor_settings['Small']['disk'])) {
					vm_flavor_newSlave_Id = "slave_vm_flavors_Small";
					choice = document.getElementById(vm_flavor_newSlave_Id);
					choice.style.color = "white";
				}
				if ((this.get('slaves_cpu_selection') == this.flavor_settings['Medium']['cpu']) && (this.get('slaves_ram_selection') == this.flavor_settings['Medium']['ram']) && (this.get('slaves_disk_selection') == this.flavor_settings['Medium']['disk'])) {
					vm_flavor_newSlave_Id = "slave_vm_flavors_Medium";
					choice = document.getElementById(vm_flavor_newSlave_Id);
					choice.style.color = "white";
				}
				if ((this.get('slaves_cpu_selection') == this.flavor_settings['Large']['cpu']) && (this.get('slaves_ram_selection') == this.flavor_settings['Large']['ram']) && (this.get('slaves_disk_selection') == this.flavor_settings['Large']['disk'])) {
	...
	message_hue_login : function() {
		...
		if (this.get('hue_message') === 'CDH') {
			var msg = {
				'msg_type' : 'warning',
				'msg_text' : 'IMPORTANT: First login in Hue browser with username : hdfs'
			};
			this.get('controllers.userWelcome').send('addMessage', msg);
		} else if (this.get('hue_message') === 'HUE') {
			var msg = {
				'msg_type' : 'warning',
				'msg_text' : 'IMPORTANT: First login in Hue browser with username : hduser'
			};
			this.get('controllers.userWelcome').send('addMessage', msg);
		}
	},
	...
	actions : {
		...
		applyLastCluster : function() {
			if (!Ember.isEmpty(this.get('last_cluster'))) {
				...
				Ember.run.later(function() {
					...
					if ((clusterdata.cluster_size <= (self.get('max_cluster_size_av').length + 1)) && ((clusterdata.cpu_master + (clusterdata.cpu_slaves * (clusterdata.cluster_size - 1))) <= self.get('cpu_available') + self.get('master_cpu_selection') + self.get('slaves_cpu_selection') * (self.size_of_cluster() - 1)) && ((clusterdata.ram_master + (clusterdata.ram_slaves * (clusterdata.cluster_size - 1))) <= self.get('ram_available') + self.get('master_ram_selection') + self.get('slaves_ram_selection') * (self.size_of_cluster() - 1)) && ((clusterdata.disk_master + (clusterdata.disk_slaves * (clusterdata.cluster_size - 1))) <= self.get('disk_available') + self.get('master_disk_selection') + self.get('slaves_disk_selection') * (self.size_of_cluster() - 1))) {
		...
		findLastCluster : function() {
			...
			store.fetch('user', 1).then(function(user) {
				...
				if (!Ember.isEmpty(self.get('last_cluster'))) {
					...
					var label = '<b>Projects</b>: <span class="text text-info">' + clusterdata.project_name + '</span>' + '<br><b>Available Images</b>: <span class="text text-info">' + clusterdata.os_image + '</span>' + '<br><b>Cluster Size</b>: <span class="text text-info">' + clusterdata.cluster_size + '</span>' + '<br><b>Storage</b>: <span class="text text-info">' + self.get('reverse_storage_lookup')[clusterdata.disk_template] + '</span>' + '<br><b>Master CPUs</b>: <span class="text text-info">' + clusterdata.cpu_master + '</span>' + '<br><b>Master RAM</b>: <span class="text text-info">' + clusterdata.ram_master + '</span>' + '<br><b>Master Disk Size</b>: <span class="text text-info">' + clusterdata.disk_master + '</span>' + '<br><b>Slaves CPUs</b>: <span class="text text-info">' + clusterdata.cpu_slaves + '</span>' + '<br><b>Slaves RAM</b>: <span class="text text-info">' + clusterdata.ram_slaves + '</span>' + '<br><b>Slaves Disk Size</b>: <span class="text text-info">' + clusterdata.disk_slaves + '</span>';
		...
		disk_template_selection : function(value, name) {
			...
			if (value == 'Standard') {
				var length = this.get('content').objectAt(this.get('project_index')).get('disk_choices').length;
		...
		go_to_create : function() {
			...
			if (!Ember.isBlank(this.get('alert_mes_network')) || !Ember.isBlank(this.get('alert_mes_float_ip'))) {
				...
				if ((this.get('total_cpu_selection') <= this.get('content').objectAt(this.get('project_index')).get('cpu_av')) && (this.get('total_ram_selection') <= this.get('content').objectAt(this.get('project_index')).get('ram_av')) && (this.get('total_disk_selection') <= this.get('content').objectAt(this.get('project_index')).get('disk_av')) && (this.get('content').objectAt(this.get('project_index')).get('net_av') > 0) && (this.get('content').objectAt(this.get('project_index')).get('floatip_av') > 0)) {
					var self = this;
					// PUT request
